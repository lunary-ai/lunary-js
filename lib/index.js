...
var LLMonitor = class {
  ...
  async trackEvent(type, event, data) {
    ...
    const context = context_default.tryUse();
    const eventData = {
      event,
      type,
  ...
  wrap(type, func, params) {
    const wrappedFn = /* @__PURE__ */ __name((...args) => {
      let executeOriginalPromise = /* @__PURE__ */ __name(() => this.executeWrappedFunction(type, func, args, params), "executeOriginalPromise");
      const result = {
        then: (onFulfilled, onRejected) => executeOriginalPromise().then(onFulfilled, onRejected),
  ...
  async executeWrappedFunction(type, func, args, params) {
    ...
    try {
      const currentContext = context_default.tryUse();
      const context = {
        parentRunId: runId,
      ...
      const output = await context_default.callAsync(context, async () => {
        return func(...args);
      });
  ...
  async executeIdentifiedWrappedFunction(type, func, args, userId, userProps, params) {
    const currentContext = context_default.tryUse();
    const context = {
    ...
    return context_default.callAsync(context, async () => {
