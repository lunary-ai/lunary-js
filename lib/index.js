...
function monitorLangchainLLM(chat, llmonitor2, tags) {
  ...
  chat.generate = llmonitor2.wrapModel(originalGenerate, {
    name,
...
function monitorLangchainTool(tool, llmonitor2, tags) {
  const originalCall = tool.call;
  tool.call = llmonitor2.wrapTool(originalCall, {
    name: tool.name,
...
function monitorOpenAi(openai, llmonitor2, tags) {
  ...
  openai.createChatCompletion = llmonitor2.wrapModel(
  ...
  monitor(entities, { tags } = {}) {
    ...
    entities.forEach((entity) => {
      ...
      if (parentName === "Tool" || parentName === "StructuredTool") {
        monitorLangchainTool(entity, llmonitor2, tags);
        return;
      }
    });
  }
