...
var Lunary = class {
  ...
  getRawTemplate = async (templateSlug) => {
   ...
   */
  renderTemplate = async (templateSlug, data) => {
    const { slug, content, extra, mode } = await this.getRawTemplate(
      templateSlug
    );
    const rendered = typeof content === "string" ? Mustache.render(content, data) : content.map((t) => Mustache.render(t, data));
    return {
      ...extra,
