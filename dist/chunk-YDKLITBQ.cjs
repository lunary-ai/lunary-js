...
function getEnvironmentVariable2(name) {
  try {
    return typeof process !== "undefined" ? (
      // eslint-disable-next-line no-process-env
      _optionalChain([process, 'access', _112 => _112.env, 'optionalAccess', _113 => _113[name]])
    ) : void 0;
  } catch (e) {
    return void 0;
  }
...
var BaseRunManager = class {
  ...
  async handleText(text) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      try {
        await _optionalChain([handler, 'access', _114 => _114.handleText, 'optionalCall', _115 => _115(text, this.runId, this._parentRunId, this.tags)]);
      } catch (err) {
...
var CallbackManagerForRetrieverRun = class extends BaseRunManager {
  ...
  async handleRetrieverEnd(documents) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreRetriever) {
        try {
          await _optionalChain([handler, 'access', _116 => _116.handleRetrieverEnd, 'optionalCall', _117 => _117(documents, this.runId, this._parentRunId, this.tags)]);
        } catch (err) {
  ...
  async handleRetrieverError(err) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreRetriever) {
        try {
          await _optionalChain([handler, 'access', _118 => _118.handleRetrieverError, 'optionalCall', _119 => _119(err, this.runId, this._parentRunId, this.tags)]);
        } catch (error) {
...
var CallbackManagerForLLMRun = class extends BaseRunManager {
  ...
  async handleLLMNewToken(token, idx, _runId, _parentRunId, _tags, fields) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreLLM) {
        try {
          await _optionalChain([handler, 'access', _120 => _120.handleLLMNewToken, 'optionalCall', _121 => _121(token, _nullishCoalesce(idx, () => ( { prompt: 0, completion: 0 })), this.runId, this._parentRunId, this.tags, fields)]);
        } catch (err) {
  ...
  async handleLLMError(err) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreLLM) {
        try {
          await _optionalChain([handler, 'access', _122 => _122.handleLLMError, 'optionalCall', _123 => _123(err, this.runId, this._parentRunId, this.tags)]);
        } catch (err2) {
  ...
  async handleLLMEnd(output) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreLLM) {
        try {
          await _optionalChain([handler, 'access', _124 => _124.handleLLMEnd, 'optionalCall', _125 => _125(output, this.runId, this._parentRunId, this.tags)]);
        } catch (err) {
...
var CallbackManagerForChainRun = class extends BaseRunManager {
  ...
  async handleChainError(err, _runId, _parentRunId, _tags, kwargs) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreChain) {
        try {
          await _optionalChain([handler, 'access', _126 => _126.handleChainError, 'optionalCall', _127 => _127(err, this.runId, this._parentRunId, this.tags, kwargs)]);
        } catch (err2) {
  ...
  async handleChainEnd(output, _runId, _parentRunId, _tags, kwargs) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreChain) {
        try {
          await _optionalChain([handler, 'access', _128 => _128.handleChainEnd, 'optionalCall', _129 => _129(output, this.runId, this._parentRunId, this.tags, kwargs)]);
        } catch (err) {
  ...
  async handleAgentAction(action) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreAgent) {
        try {
          await _optionalChain([handler, 'access', _130 => _130.handleAgentAction, 'optionalCall', _131 => _131(action, this.runId, this._parentRunId, this.tags)]);
        } catch (err) {
  ...
  async handleAgentEnd(action) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreAgent) {
        try {
          await _optionalChain([handler, 'access', _132 => _132.handleAgentEnd, 'optionalCall', _133 => _133(action, this.runId, this._parentRunId, this.tags)]);
        } catch (err) {
...
var CallbackManagerForToolRun = class extends BaseRunManager {
  ...
  async handleToolError(err) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreAgent) {
        try {
          await _optionalChain([handler, 'access', _134 => _134.handleToolError, 'optionalCall', _135 => _135(err, this.runId, this._parentRunId, this.tags)]);
        } catch (err2) {
  ...
  async handleToolEnd(output) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreAgent) {
        try {
          await _optionalChain([handler, 'access', _136 => _136.handleToolEnd, 'optionalCall', _137 => _137(output, this.runId, this._parentRunId, this.tags)]);
        } catch (err) {
...
var CallbackManager = class _CallbackManager extends BaseCallbackManager {
  ...
  constructor(parentRunId, options) {
    ...
    this.handlers = _nullishCoalesce(_optionalChain([options, 'optionalAccess', _138 => _138.handlers]), () => ( this.handlers));
    this.inheritableHandlers = _nullishCoalesce(_optionalChain([options, 'optionalAccess', _139 => _139.inheritableHandlers]), () => ( this.inheritableHandlers));
    this.tags = _nullishCoalesce(_optionalChain([options, 'optionalAccess', _140 => _140.tags]), () => ( this.tags));
    this.inheritableTags = _nullishCoalesce(_optionalChain([options, 'optionalAccess', _141 => _141.inheritableTags]), () => ( this.inheritableTags));
    this.metadata = _nullishCoalesce(_optionalChain([options, 'optionalAccess', _142 => _142.metadata]), () => ( this.metadata));
    this.inheritableMetadata = _nullishCoalesce(_optionalChain([options, 'optionalAccess', _143 => _143.inheritableMetadata]), () => ( this.inheritableMetadata));
  ...
  async handleLLMStart(llm, prompts, _runId = void 0, _parentRunId = void 0, extraParams = void 0, _tags = void 0, _metadata = void 0, runName = void 0) {
    return Promise.all(prompts.map(async (prompt) => {
      ...
      await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
        if (!handler.ignoreLLM) {
          try {
            await _optionalChain([handler, 'access', _144 => _144.handleLLMStart, 'optionalCall', _145 => _145(llm, [prompt], runId, this._parentRunId, extraParams, this.tags, this.metadata, runName)]);
          } catch (err) {
        ...
        if (!handler.ignoreLLM) {
          try {
            if (handler.handleChatModelStart) {
              await _optionalChain([handler, 'access', _146 => _146.handleChatModelStart, 'optionalCall', _147 => _147(llm, [messageGroup], runId, this._parentRunId, extraParams, this.tags, this.metadata, runName)]);
            } else if (handler.handleLLMStart) {
              const messageString = getBufferString(messageGroup);
              await _optionalChain([handler, 'access', _148 => _148.handleLLMStart, 'optionalCall', _149 => _149(llm, [messageString], runId, this._parentRunId, extraParams, this.tags, this.metadata, runName)]);
            }
          } catch (err) {
            console.error(`Error in handler ${handler.constructor.name}, handleLLMStart: ${err}`);
          }
  ...
  async handleChainStart(chain, inputs, runId = v4_default(), runType = void 0, _tags = void 0, _metadata = void 0, runName = void 0) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreChain) {
        try {
          await _optionalChain([handler, 'access', _150 => _150.handleChainStart, 'optionalCall', _151 => _151(chain, inputs, runId, this._parentRunId, this.tags, this.metadata, runType, runName)]);
        } catch (err) {
  ...
  async handleToolStart(tool, input, runId = v4_default(), _parentRunId = void 0, _tags = void 0, _metadata = void 0, runName = void 0) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreAgent) {
        try {
          await _optionalChain([handler, 'access', _152 => _152.handleToolStart, 'optionalCall', _153 => _153(tool, input, runId, this._parentRunId, this.tags, this.metadata, runName)]);
        } catch (err) {
  ...
  async handleRetrieverStart(retriever, query, runId = v4_default(), _parentRunId = void 0, _tags = void 0, _metadata = void 0, runName = void 0) {
    await Promise.all(this.handlers.map((handler) => consumeCallback(async () => {
      if (!handler.ignoreRetriever) {
        try {
          await _optionalChain([handler, 'access', _154 => _154.handleRetrieverStart, 'optionalCall', _155 => _155(retriever, query, runId, this._parentRunId, this.tags, this.metadata, runName)]);
        } catch (err) {
  ...
  static async configure(inheritableHandlers, localHandlers, inheritableTags, localTags, inheritableMetadata, localMetadata, options) {
    ...
    if (inheritableHandlers || localHandlers) {
      if (Array.isArray(inheritableHandlers) || !inheritableHandlers) {
        ...
        callbackManager.setHandlers(_nullishCoalesce(_optionalChain([inheritableHandlers, 'optionalAccess', _156 => _156.map, 'call', _157 => _157(ensureHandler)]), () => ( [])), true);
      } else {
        callbackManager = inheritableHandlers;
      }
      callbackManager = callbackManager.copy(Array.isArray(localHandlers) ? localHandlers.map(ensureHandler) : _optionalChain([localHandlers, 'optionalAccess', _158 => _158.handlers]), false);
    }
    const verboseEnabled = getEnvironmentVariable2("LANGCHAIN_VERBOSE") === "true" || _optionalChain([options, 'optionalAccess', _159 => _159.verbose]);
...
async function _getStandardizedInputs(run, schemaFormat) {
  ...
  if (Object.keys(inputs).length === 1 && _optionalChain([inputs, 'optionalAccess', _160 => _160.input]) === "") {
    return void 0;
  }
  return inputs.input;
}
...
async function _getStandardizedOutputs(run, schemaFormat) {
  ...
  if (outputs !== void 0 && Object.keys(outputs).length === 1 && _optionalChain([outputs, 'optionalAccess', _161 => _161.output]) !== void 0) {
...
var LogStreamCallbackHandler = class extends BaseTracer {
  ...
  constructor(fields) {
    ...
    this.autoClose = _nullishCoalesce(_optionalChain([fields, 'optionalAccess', _162 => _162.autoClose]), () => ( true));
    this.includeNames = _optionalChain([fields, 'optionalAccess', _163 => _163.includeNames]);
    this.includeTypes = _optionalChain([fields, 'optionalAccess', _164 => _164.includeTypes]);
    this.includeTags = _optionalChain([fields, 'optionalAccess', _165 => _165.includeTags]);
    this.excludeNames = _optionalChain([fields, 'optionalAccess', _166 => _166.excludeNames]);
    this.excludeTypes = _optionalChain([fields, 'optionalAccess', _167 => _167.excludeTypes]);
    this.excludeTags = _optionalChain([fields, 'optionalAccess', _168 => _168.excludeTags]);
    this._schemaFormat = _nullishCoalesce(_optionalChain([fields, 'optionalAccess', _169 => _169._schemaFormat]), () => ( this._schemaFormat));
  ...
  _includeRun(run) {
    ...
    if (this.includeTags !== void 0) {
      include = include || runTags.find((tag) => _optionalChain([this, 'access', _170 => _170.includeTags, 'optionalAccess', _171 => _171.includes, 'call', _172 => _172(tag)])) !== void 0;
    }
    ...
    if (this.excludeTags !== void 0) {
      include = include && runTags.every((tag) => !_optionalChain([this, 'access', _173 => _173.excludeTags, 'optionalAccess', _174 => _174.includes, 'call', _175 => _175(tag)]));
    }
    return include;
  }
  ...
  async onRunCreate(run) {
    ...
    const logEntry = {
      ...
      metadata: _nullishCoalesce(_optionalChain([run, 'access', _176 => _176.extra, 'optionalAccess', _177 => _177.metadata]), () => ( {})),
  ...
  async onLLMNewToken(run, token, kwargs) {
    ...
    if (isChatModel) {
      if (isChatGenerationChunk(_optionalChain([kwargs, 'optionalAccess', _178 => _178.chunk]))) {
        streamedOutputValue = _optionalChain([kwargs, 'optionalAccess', _179 => _179.chunk]);
      } else {
...
async function getCallbackManagerForConfig(config) {
  return CallbackManager.configure(_optionalChain([config, 'optionalAccess', _180 => _180.callbacks]), void 0, _optionalChain([config, 'optionalAccess', _181 => _181.tags]), void 0, _optionalChain([config, 'optionalAccess', _182 => _182.metadata]));
}
...
function ensureConfig(config) {
  ...
  if (_optionalChain([loadedConfig, 'optionalAccess', _183 => _183.configurable])) {
    for (const key of Object.keys(loadedConfig.configurable)) {
      if (PRIMITIVES.has(typeof loadedConfig.configurable[key]) && !_optionalChain([empty, 'access', _184 => _184.metadata, 'optionalAccess', _185 => _185[key]])) {
        if (!empty.metadata) {
          empty.metadata = {};
        }
...
var defaultFailedAttemptHandler = /* @__PURE__ */ _chunkEC6JY3PVcjs.__name.call(void 0, (error) => {
  ...
  if (_optionalChain([error, 'optionalAccess', _186 => _186.code]) === "ECONNABORTED") {
    throw error;
  }
  const status = (
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    _nullishCoalesce(_optionalChain([error, 'optionalAccess', _187 => _187.response, 'optionalAccess', _188 => _188.status]), () => ( _optionalChain([error, 'optionalAccess', _189 => _189.status])))
  );
  if (status && STATUS_NO_RETRY2.includes(+status)) {
    throw error;
  }
  if (_optionalChain([error, 'optionalAccess', _190 => _190.error, 'optionalAccess', _191 => _191.code]) === "insufficient_quota") {
    const err = new Error(_optionalChain([error, 'optionalAccess', _192 => _192.message]));
  ...
  callWithOptions(options, callable, ...args) {
    if (options.signal) {
      return Promise.race([
        this.call(callable, ...args),
        new Promise((_, reject) => {
          _optionalChain([options, 'access', _193 => _193.signal, 'optionalAccess', _194 => _194.addEventListener, 'call', _195 => _195("abort", () => {
            reject(new Error("AbortError"));
          })]);
        })
      ]);
...
var _RootEventFilter = class {
  ...
  includeEvent(event, rootType) {
    ...
    if (this.includeTags !== void 0) {
      include = include || eventTags.some((tag) => _optionalChain([this, 'access', _196 => _196.includeTags, 'optionalAccess', _197 => _197.includes, 'call', _198 => _198(tag)]));
    }
    if (this.excludeNames !== void 0) {
    ...
    if (this.excludeTags !== void 0) {
      include = include && eventTags.every((tag) => !_optionalChain([this, 'access', _199 => _199.excludeTags, 'optionalAccess', _200 => _200.includes, 'call', _201 => _201(tag)]));
    }
    return include;
  }
};
...
var Runnable = class extends Serializable {
  ...
  withRetry(fields) {
    return new RunnableRetry({
      ...
      kwargs: {},
      config: {},
      maxAttemptNumber: _optionalChain([fields, 'optionalAccess', _202 => _202.stopAfterAttempt]),
      ...fields
    });
  }
  /**
  ...
  async batch(inputs, options, batchOptions) {
    ...
    const maxConcurrency = _nullishCoalesce(_optionalChain([configList, 'access', _203 => _203[0], 'optionalAccess', _204 => _204.maxConcurrency]), () => ( _optionalChain([batchOptions, 'optionalAccess', _205 => _205.maxConcurrency])));
    ...
    const batchCalls = inputs.map((input, i) => caller.call(async () => {
      try {
        ...
        if (_optionalChain([batchOptions, 'optionalAccess', _206 => _206.returnExceptions])) {
          return e;
        }
        throw e;
      }
  ...
  async _callWithConfig(func, input, options) {
    ...
    const runManager = await _optionalChain([callbackManager_, 'optionalAccess', _207 => _207.handleChainStart, 'call', _208 => _208(this.toJSON(), _coerceToDict2(input, "input"), void 0, _optionalChain([config, 'optionalAccess', _209 => _209.runType]), void 0, void 0, _nullishCoalesce(_optionalChain([config, 'optionalAccess', _210 => _210.runName]), () => ( this.getName())))]);
    let output;
    try {
      output = await func.call(this, input, config, runManager);
    } catch (e) {
      await _optionalChain([runManager, 'optionalAccess', _211 => _211.handleChainError, 'call', _212 => _212(e)]);
      throw e;
    }
    await _optionalChain([runManager, 'optionalAccess', _213 => _213.handleChainEnd, 'call', _214 => _214(_coerceToDict2(output, "output"))]);
    return output;
  }
  /**
  ...
  async _batchWithConfig(func, inputs, options, batchOptions) {
    ...
    const runManagers = await Promise.all(callbackManagers.map((callbackManager, i) => _optionalChain([callbackManager, 'optionalAccess', _215 => _215.handleChainStart, 'call', _216 => _216(this.toJSON(), _coerceToDict2(inputs[i], "input"), void 0, optionsList[i].runType, void 0, void 0, _nullishCoalesce(optionsList[i].runName, () => ( this.getName())))])));
    let outputs;
    try {
      outputs = await func.call(this, inputs, optionsList, runManagers, batchOptions);
    } catch (e) {
      await Promise.all(runManagers.map((runManager) => _optionalChain([runManager, 'optionalAccess', _217 => _217.handleChainError, 'call', _218 => _218(e)])));
      throw e;
    }
    await Promise.all(runManagers.map((runManager) => _optionalChain([runManager, 'optionalAccess', _219 => _219.handleChainEnd, 'call', _220 => _220(_coerceToDict2(outputs, "output"))])));
    return outputs;
  }
  /**
  ...
  async *_transformStreamWithConfig(inputGenerator, transformer, options) {
    ...
    let runManager;
    try {
      const pipe = await pipeGeneratorWithSetup(transformer.bind(this), wrapInputForTracing(), async () => _optionalChain([callbackManager_, 'optionalAccess', _221 => _221.handleChainStart, 'call', _222 => _222(this.toJSON(), { input: "" }, void 0, _optionalChain([config, 'optionalAccess', _223 => _223.runType]), void 0, void 0, _nullishCoalesce(_optionalChain([config, 'optionalAccess', _224 => _224.runName]), () => ( this.getName())))]), config);
      ...
      const streamLogHandler = _optionalChain([runManager, 'optionalAccess', _225 => _225.handlers, 'access', _226 => _226.find, 'call', _227 => _227(isLogStreamHandler)]);
      ...
      await _optionalChain([runManager, 'optionalAccess', _228 => _228.handleChainError, 'call', _229 => _229(e, void 0, void 0, void 0, {
        inputs: _coerceToDict2(finalInput, "input")
      })]);
      throw e;
    }
    await _optionalChain([runManager, 'optionalAccess', _230 => _230.handleChainEnd, 'call', _231 => _231(_nullishCoalesce(finalOutput, () => ( {})), void 0, void 0, void 0, { inputs: _coerceToDict2(finalInput, "input") })]);
  }
  /**
  ...
  async *streamEvents(input, options, streamOptions) {
    ...
    const state = _optionalChain([runLog, 'optionalAccess', _232 => _232.state]);
    if (state !== void 0) {
      const event = {
...
var RunnableEach = class _RunnableEach extends Runnable {
  ...
  async _invoke(inputs, config, runManager) {
    return this.bound.batch(inputs, patchConfig(config, { callbacks: _optionalChain([runManager, 'optionalAccess', _233 => _233.getChild, 'call', _234 => _234()]) }));
  }
  /**
   * Bind lifecycle listeners to a Runnable, returning a new Runnable.
   * The Run object contains information about the run, including its id,
...
var RunnableRetry = class extends RunnableBinding {
  ...
  _patchConfigForRetry(attempt, config, runManager) {
    ...
    return patchConfig(config, { callbacks: _optionalChain([runManager, 'optionalAccess', _235 => _235.getChild, 'call', _236 => _236(tag)]) });
  }
  ...
  async _batch(inputs, configs, runManagers, batchOptions) {
    ...
    try {
      await (0, import_p_retry3.default)(async (attemptNumber) => {
        ...
        const patchedConfigs = remainingIndexes.map((i) => this._patchConfigForRetry(attemptNumber, _optionalChain([configs, 'optionalAccess', _237 => _237[i]]), _optionalChain([runManagers, 'optionalAccess', _238 => _238[i]])));
      ...
      if (_optionalChain([batchOptions, 'optionalAccess', _239 => _239.returnExceptions]) !== true) {
        throw e;
      }
    }
...
var RunnableSequence = class _RunnableSequence extends Runnable {
  ...
  async invoke(input, options) {
    const config = ensureConfig(options);
    const callbackManager_ = await getCallbackManagerForConfig(config);
    const runManager = await _optionalChain([callbackManager_, 'optionalAccess', _240 => _240.handleChainStart, 'call', _241 => _241(this.toJSON(), _coerceToDict2(input, "input"), void 0, void 0, void 0, void 0, _optionalChain([config, 'optionalAccess', _242 => _242.runName]))]);
    ...
    try {
      ...
      for (let i = 0; i < initialSteps.length; i += 1) {
        ...
        nextStepInput = await step.invoke(nextStepInput, patchConfig(config, {
          callbacks: _optionalChain([runManager, 'optionalAccess', _243 => _243.getChild, 'call', _244 => _244(`seq:step:${i + 1}`)])
        }));
      }
      finalOutput = await this.last.invoke(nextStepInput, patchConfig(config, {
        callbacks: _optionalChain([runManager, 'optionalAccess', _245 => _245.getChild, 'call', _246 => _246(`seq:step:${this.steps.length}`)])
      }));
    } catch (e) {
      await _optionalChain([runManager, 'optionalAccess', _247 => _247.handleChainError, 'call', _248 => _248(e)]);
      throw e;
    }
    await _optionalChain([runManager, 'optionalAccess', _249 => _249.handleChainEnd, 'call', _250 => _250(_coerceToDict2(finalOutput, "output"))]);
    return finalOutput;
  }
  async batch(inputs, options, batchOptions) {
    const configList = this._getOptionsList(_nullishCoalesce(options, () => ( {})), inputs.length);
    const callbackManagers = await Promise.all(configList.map(getCallbackManagerForConfig));
    const runManagers = await Promise.all(callbackManagers.map((callbackManager, i) => _optionalChain([callbackManager, 'optionalAccess', _251 => _251.handleChainStart, 'call', _252 => _252(this.toJSON(), _coerceToDict2(inputs[i], "input"), void 0, void 0, void 0, void 0, configList[i].runName)])));
    let nextStepInputs = inputs;
    try {
      for (let i = 0; i < this.steps.length; i += 1) {
        const step = this.steps[i];
        nextStepInputs = await step.batch(nextStepInputs, runManagers.map((runManager, j) => {
          const childRunManager = _optionalChain([runManager, 'optionalAccess', _253 => _253.getChild, 'call', _254 => _254(`seq:step:${i + 1}`)]);
          return patchConfig(configList[j], { callbacks: childRunManager });
        }), batchOptions);
      }
    } catch (e) {
      await Promise.all(runManagers.map((runManager) => _optionalChain([runManager, 'optionalAccess', _255 => _255.handleChainError, 'call', _256 => _256(e)])));
      throw e;
    }
    await Promise.all(runManagers.map((runManager) => _optionalChain([runManager, 'optionalAccess', _257 => _257.handleChainEnd, 'call', _258 => _258(_coerceToDict2(nextStepInputs, "output"))])));
    return nextStepInputs;
  }
  async *_streamIterator(input, options) {
    const callbackManager_ = await getCallbackManagerForConfig(options);
    const runManager = await _optionalChain([callbackManager_, 'optionalAccess', _259 => _259.handleChainStart, 'call', _260 => _260(this.toJSON(), _coerceToDict2(input, "input"), void 0, void 0, void 0, void 0, _optionalChain([options, 'optionalAccess', _261 => _261.runName]))]);
    ...
    try {
      let finalGenerator = steps[0].transform(inputGenerator(), patchConfig(options, {
        callbacks: _optionalChain([runManager, 'optionalAccess', _262 => _262.getChild, 'call', _263 => _263(`seq:step:1`)])
      }));
      for (let i = 1; i < steps.length; i += 1) {
        const step = steps[i];
        finalGenerator = await step.transform(finalGenerator, patchConfig(options, {
          callbacks: _optionalChain([runManager, 'optionalAccess', _264 => _264.getChild, 'call', _265 => _265(`seq:step:${i + 1}`)])
        }));
      }
      ...
      await _optionalChain([runManager, 'optionalAccess', _266 => _266.handleChainError, 'call', _267 => _267(e)]);
      throw e;
    }
    await _optionalChain([runManager, 'optionalAccess', _268 => _268.handleChainEnd, 'call', _269 => _269(_coerceToDict2(finalOutput, "output"))]);
  }
  pipe(coerceable) {
...
var RunnableMap = class _RunnableMap extends Runnable {
  ...
  async invoke(input, options) {
    const config = ensureConfig(options);
    const callbackManager_ = await getCallbackManagerForConfig(config);
    const runManager = await _optionalChain([callbackManager_, 'optionalAccess', _270 => _270.handleChainStart, 'call', _271 => _271(this.toJSON(), {
      input
    }, void 0, void 0, void 0, void 0, _optionalChain([config, 'optionalAccess', _272 => _272.runName]))]);
    const output = {};
    try {
      await Promise.all(Object.entries(this.steps).map(async ([key, runnable]) => {
        output[key] = await runnable.invoke(input, patchConfig(config, {
          callbacks: _optionalChain([runManager, 'optionalAccess', _273 => _273.getChild, 'call', _274 => _274(`map:key:${key}`)])
        }));
      }));
    } catch (e) {
      await _optionalChain([runManager, 'optionalAccess', _275 => _275.handleChainError, 'call', _276 => _276(e)]);
      throw e;
    }
    await _optionalChain([runManager, 'optionalAccess', _277 => _277.handleChainEnd, 'call', _278 => _278(output)]);
    return output;
  }
  async *_transform(generator, runManager, options) {
    ...
    const tasks = new Map(Object.entries(steps).map(([key, runnable], i) => {
      const gen = runnable.transform(inputCopies[i], patchConfig(options, {
        callbacks: _optionalChain([runManager, 'optionalAccess', _279 => _279.getChild, 'call', _280 => _280(`map:key:${key}`)])
      }));
...
var RunnableLambda = class _RunnableLambda extends Runnable {
  ...
  async _invoke(input, config, runManager) {
    return new Promise((resolve, reject) => {
      const childConfig = patchConfig(config, {
        callbacks: _optionalChain([runManager, 'optionalAccess', _281 => _281.getChild, 'call', _282 => _282()]),
        recursionLimit: (_nullishCoalesce(_optionalChain([config, 'optionalAccess', _283 => _283.recursionLimit]), () => ( DEFAULT_RECURSION_LIMIT))) - 1
      });
      void AsyncLocalStorageProviderSingleton.getInstance().run(childConfig, async () => {
        try {
          ...
          if (output && Runnable.isRunnable(output)) {
            if (_optionalChain([config, 'optionalAccess', _284 => _284.recursionLimit]) === 0) {
  ...
  async *_transform(generator, runManager, config) {
    ...
    if (output && Runnable.isRunnable(output)) {
      if (_optionalChain([config, 'optionalAccess', _285 => _285.recursionLimit]) === 0) {
        throw new Error("Recursion limit reached.");
      }
      const stream = await output.stream(finalChunk, patchConfig(config, {
        callbacks: _optionalChain([runManager, 'optionalAccess', _286 => _286.getChild, 'call', _287 => _287()]),
        recursionLimit: (_nullishCoalesce(_optionalChain([config, 'optionalAccess', _288 => _288.recursionLimit]), () => ( DEFAULT_RECURSION_LIMIT))) - 1
      }));
...
var RunnableWithFallbacks = class extends Runnable {
  ...
  *runnables() {
  ...
  async invoke(input, options) {
    const callbackManager_ = await CallbackManager.configure(_optionalChain([options, 'optionalAccess', _289 => _289.callbacks]), void 0, _optionalChain([options, 'optionalAccess', _290 => _290.tags]), void 0, _optionalChain([options, 'optionalAccess', _291 => _291.metadata]));
    const runManager = await _optionalChain([callbackManager_, 'optionalAccess', _292 => _292.handleChainStart, 'call', _293 => _293(this.toJSON(), _coerceToDict2(input, "input"), void 0, void 0, void 0, void 0, _optionalChain([options, 'optionalAccess', _294 => _294.runName]))]);
    let firstError;
    for (const runnable of this.runnables()) {
      try {
        const output = await runnable.invoke(input, patchConfig(options, { callbacks: _optionalChain([runManager, 'optionalAccess', _295 => _295.getChild, 'call', _296 => _296()]) }));
        await _optionalChain([runManager, 'optionalAccess', _297 => _297.handleChainEnd, 'call', _298 => _298(_coerceToDict2(output, "output"))]);
        return output;
      } catch (e) {
    ...
    await _optionalChain([runManager, 'optionalAccess', _299 => _299.handleChainError, 'call', _300 => _300(firstError)]);
    throw firstError;
  }
  async batch(inputs, options, batchOptions) {
    if (_optionalChain([batchOptions, 'optionalAccess', _301 => _301.returnExceptions])) {
      throw new Error("Not implemented.");
    }
    const configList = this._getOptionsList(_nullishCoalesce(options, () => ( {})), inputs.length);
    const callbackManagers = await Promise.all(configList.map((config) => CallbackManager.configure(_optionalChain([config, 'optionalAccess', _302 => _302.callbacks]), void 0, _optionalChain([config, 'optionalAccess', _303 => _303.tags]), void 0, _optionalChain([config, 'optionalAccess', _304 => _304.metadata]))));
    const runManagers = await Promise.all(callbackManagers.map((callbackManager, i) => _optionalChain([callbackManager, 'optionalAccess', _305 => _305.handleChainStart, 'call', _306 => _306(this.toJSON(), _coerceToDict2(inputs[i], "input"), void 0, void 0, void 0, void 0, configList[i].runName)])));
    let firstError;
    for (const runnable of this.runnables()) {
      try {
        const outputs = await runnable.batch(inputs, runManagers.map((runManager, j) => patchConfig(configList[j], {
          callbacks: _optionalChain([runManager, 'optionalAccess', _307 => _307.getChild, 'call', _308 => _308()])
        })), batchOptions);
        await Promise.all(runManagers.map((runManager, i) => _optionalChain([runManager, 'optionalAccess', _309 => _309.handleChainEnd, 'call', _310 => _310(_coerceToDict2(outputs[i], "output"))])));
        return outputs;
      } catch (e) {
        if (firstError === void 0) {
          firstError = e;
    ...
    await Promise.all(runManagers.map((runManager) => _optionalChain([runManager, 'optionalAccess', _311 => _311.handleChainError, 'call', _312 => _312(firstError)])));
    throw firstError;
  }
};
...
var RunnableAssign = class extends Runnable {
  ...
  async *_transform(generator, runManager, options) {
    ...
    const mapperOutput = this.mapper.transform(forMapper, patchConfig(options, { callbacks: _optionalChain([runManager, 'optionalAccess', _313 => _313.getChild, 'call', _314 => _314()]) }));
...
var BasePromptTemplate = class extends Runnable {
  ...
  static async deserialize(data) {
    switch (data._type) {
      case "prompt": {
        const { PromptTemplate: PromptTemplate2 } = await Promise.resolve().then(() => _interopRequireWildcard(require("./prompt-W6TW72PF.cjs")));
      ...
      case "few_shot": {
        const { FewShotPromptTemplate } = await Promise.resolve().then(() => _interopRequireWildcard(require("./few_shot-3CALKWTG.cjs")));
