...
var Lunary = (_class2 = class {
  ...
  __init8() {this.getRawTemplate = async (templateSlug) => {
   ...
   */
  __init9() {this.renderTemplate = async (templateSlug, data) => {
    const { slug, content, extra, mode } = await this.getRawTemplate(
      templateSlug
    );
    const rendered = typeof content === "string" ? _mustache2.default.render(content, data) : content.map((t) => _mustache2.default.render(t, data));
    return {
      ...extra,
